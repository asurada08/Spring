...
public class BDAO {
	...
	public void write(final String bName, final String bSubject, final String bContent) {
		String sql = "insert into mvc_bbs(bNo, bName, bSubject, bContent, bHit, bGroup, bStep, bIndent) values(seq_bbs.nextval, ?, ?, ?, 0, seq_bbs.currval, 0, 0)";
	...
	public BVO contentView(String bNo) {
		addHit(bNo);
		
		String sql ="select * from mvc_bbs where bNo ="+bNo;
	...
	public BVO reqlyForm(String bNo) {
		String sql = "select * from mvc_bbs where bNo = " + bNo;
		return this.template.queryForObject(sql, new BeanPropertyRowMapper<BVO>(BVO.class));
	}//replyForm()

	public void replyOk(String bNo, final String bName, final String bSubject, final String bContent, final String bGroup, final String bStep, final String bIndent) {
	...
	private void replySet(final String group, final String step) {
